//
//  DosePreviewViewController.swift
//  CannaTrack
//
...
class DosePreviewViewController: UIViewController {
	...
	init(dose: Dose) {
		...
		imageView.image = {
			...
			if let productImage = dose.doseImage {
				...
				dose.doseImage = correctedUIImage
			} else {
		...
		preferredContentSize = dose.doseImage?.size ?? CGSize(width: 200, height: 200)
	}
